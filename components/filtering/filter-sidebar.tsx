...
export default function FilterSidebar({ categories, sellers }: FilterSidebarProps) {
  ...
  const updateFilter = useCallback((updates: Partial<SearchParams>) => {
  ...
  return (
    <div className="w-80 flex-shrink-0 p-6 space-y-6 bg-gradient-to-br from-background to-background/80 rounded-xl shadow-lg border border-primary/10 h-full mr-8 scroll-smooth scrollbar-hide">
      <div className="flex justify-between items-center">
        <h2 className="font-bold text-2xl bg-clip-text text-transparent bg-gradient-to-r from-primary to-secondary">Filters</h2>
        <Button variant="outline" size="sm" onClick={handleResetFilters} className="text-primary hover:bg-primary/10">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {categories.map((category) => (
            ...
            >
              {category.name}
              {searchParams.get('categoryId') === category.id.toString() && <Check className="w-4 h-4 ml-auto" />}
            </Button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          {sellers.map((seller: any) => (
            ...
            >
              {seller.id}
              {searchParams.get('sellerId') === seller.id.toString() && <Check className="w-4 h-4 ml-auto" />}
            </Button>
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          {['AVAILABLE', 'OUT_OF_STOCK', 'DISCONTINUED'].map((status) => (
            <Button
              ...
              size="sm"
              onClick={() => updateFilter({ status: status as "AVAILABLE" | "OUT_OF_STOCK" | "DISCONTINUED" })}
      ...
      <Button
        ...
        className={cn(
          ...
          searchParams.get('featured') === 'true' ? "bg-primary text-primary-foreground" : "hover:bg-primary/10"
        )}
        onClick={() => updateFilter({ featured: searchParams.get('featured') === 'true' ? false : true })}
      >
        <Star className="w-4 h-4 mr-2" />
        Featured Products Only
        {searchParams.get('featured') === 'true' && <Check className="w-4 h-4 ml-auto" />}
      </Button>
    </div>
  );
}